...
const BuyerRequestList = () => {
  ...
  const searchRequests = async (page = 0) => {
    ...
    try {
      ...
      const response = await apiPost(
        `/store/nego-req/search?page=${page}&size=${pageSize}`,
        searchRequest
      );
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        setTotalElements(data.totalElements);
        setRequests(data.content);
        console.log("구매자 요청 검색 결과:", data);
      } else {
        setError("구매자 요청 검색에 실패했습니다.");
      }
    } catch (err) {
      console.error("검색 API 요청 실패:", err);
      setError("구매자 요청 검색 중 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="buyerRequest-table-wrap">
        <table className="buyerRequest-table">
          <thead>
            <tr>
              <th>번호</th>
              <th>상품명</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="7" style={{textAlign: 'center', padding: '20px'}}>구매자 요청이 없습니다.</td></tr>
            ) : (
        ...
        <div className="buyerRequest-footer">
          <div className="buyerRequest-count">
            전체 {totalElements}개 요청
          </div>
          <div className="buyerRequest-pagination">
            <button 
              className="buyerRequest-page" 
              onClick={() => {
                if (currentPage > 0) {
            ...
            {[...Array(Math.max(1, totalPages))].map((_, index) => (
              <button
                key={index}
                className={`buyerRequest-page ${currentPage === index ? 'active' : ''}`}
                onClick={() => {
          ...
          <div className="buyerRequest-footer-spacer"></div>
        </div>
      </div>
    </div>
  );
